...
function SidebarItem({ service, active, onClick, dark }) {
...
function DetailPanel({ service, dark }) {
  return (
    <motion.div
      key={service.id}
...
export default function ServiceExplorer({ groupId, tone = "light" }) {
        ...
        >
          <div className="flex items-center gap-4 px-5 pb-5 pt-6 sm:px-6 lg:pb-7 lg:pt-8">
            ...
            <div>
              ...
              >
                Select a service to inspect the delivery scope.
              </p>
            </div>
          </div>
          <nav
            aria-label={`${groupId || "All"} services`}
            className="flex gap-3 overflow-x-auto px-5 pb-5 sm:px-6 lg:block lg:space-y-3 lg:overflow-visible lg:pb-6"
          >
