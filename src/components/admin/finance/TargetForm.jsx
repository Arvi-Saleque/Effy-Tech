  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div><label htmlFor="target_amount" className={labelClass}>Target amount (BDT) *</label><MoneyExpressionInput id="target_amount" name="target_amount" required defaultValue={initialData?.target_amount || ""} className={inputClass} />{errors?.target_amount?.[0] ? <p className="mt-1.5 text-xs text-rose-400">{errors.target_amount[0]}</p> : null}</div>
      ...
      <fieldset className="border-t border-neutral-800 pt-6"><legend className="mb-3 text-xs font-bold uppercase tracking-widest text-neutral-500">Target period</legend><div className="mb-5 flex gap-2 overflow-x-auto">{[["weekly", "Week"], ["monthly", "Month"], ["quarterly", "Quarter"], ["yearly", "Year"], ["custom", "Custom"]].map(([value, label]) => <button key={value} type="button" onClick={() => changePeriod(value)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-bold ${period === value ? "bg-neutral-100 text-neutral-950" : "bg-neutral-800/60 text-neutral-400"}`}>{label}</button>)}</div><div className="grid gap-5 md:grid-cols-2"><div><label htmlFor="start_date" className={labelClass}>Start date *</label><FinanceDateInput id="start_date" value={startDate} onChange={(event) => { setStartDate(event.target.value); setPeriod("custom"); }} required calendarLabel="Open target start date picker" className={inputClass} /></div><div><label htmlFor="end_date" className={labelClass}>End date *</label><FinanceDateInput id="end_date" value={endDate} onChange={(event) => { setEndDate(event.target.value); setPeriod("custom"); }} required calendarLabel="Open target end date picker" className={inputClass} />{errors?.end_date?.[0] ? <p className="mt-1.5 text-xs text-rose-400">{errors.end_date[0]}</p> : null}</div></div></fieldset>
