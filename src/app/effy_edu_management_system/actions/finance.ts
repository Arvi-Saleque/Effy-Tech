...
import {
  deleteR2File,
  generateR2UploadUrl,
} from "@/features/effy-edu-demo/lib/r2";
...
} from "@/features/effy-edu-demo/lib/validations/finance";
...
const FINANCE_PATH = "/effy_edu_management_system/teacher/finance";
...
export async function getFinanceReceiptUploadUrl(input: {
  ...
  try {
    ...
    if (
      ...
      input.sizeBytes < 1 ||
      input.sizeBytes > MAX_RECEIPT_BYTES
    ) {
      return {
        success: false,
  ...
  try {
    ...
    if (!parsed.success) {
      return {
        success: false,
        message:
          parsed.error.issues[0]?.message || "Invalid expense details.",
      };
    }
    ...
    if (error || !expense) {
      if (isFinanceReceiptPath(uploadedPath)) {
        await deleteR2File(uploadedPath!);
      }
      return {
        success: false,
...
export async function updateFinanceExpenseAction(
  ...
  try {
    ...
    if (fetchError || !oldExpense) {
      if (isFinanceReceiptPath(newUploadedPath)) {
        await deleteR2File(newUploadedPath!);
      }
    ...
    const oldReceiptWasReplaced =
      ...
      (input.removeExistingReceipt ||
        (newUploadedPath &&
          newUploadedPath !== oldExpense.receipt_storage_path));
    if (
      oldReceiptWasReplaced &&
      isFinanceReceiptPath(oldExpense.receipt_storage_path)
    ) {
  ...
  try {
    ...
    if (!parsed.success) {
      return {
        success: false,
        message:
          parsed.error.issues[0]?.message || "A void reason is required.",
      };
    }
    ...
    if (oldExpense.status === "VOID") {
      return {
        success: false,
        message: "This expense is already voided.",
      };
    }
...
export async function restoreFinanceExpenseAction(expenseId: string) {
  try {
    ...
    if (oldExpense.status !== "VOID") {
      return {
        success: false,
        message: "This expense is already posted.",
      };
    }
    ...
    if (error || !expense) {
      return {
        success: false,
        message: error?.message || "Failed to restore expense.",
      };
    }
