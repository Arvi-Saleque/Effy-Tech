...
export default function NewsEventDetailClient({
  ...
  const handleShareWhatsApp = () => {
    if (typeof window !== "undefined") {
      const text = encodeURIComponent(`Check out this update from EduPilot Coaching Academy: ${item.title} \n${window.location.href}`);
    ...
    <div className="min-h-screen bg-[#FFF9F2] pt-24 pb-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <InnerPageHero
          ...
          breadcrumbs={[
    { label: "Home", href: "/effy_edu_management_system" },
            { label: "News & Events", href: "/effy_edu_management_system/news-events" },
            { label: item.category || "Detail View" },
          ]}
          imageSrc={item.imageUrl || "/effy_edu_management_system/images/gallery-event.png"}
        />
      </div>
            ...
            {relatedItems.length > 0 && (
              <div className="bg-white border border-[#E8DDBF]/80 rounded-3xl p-6 sm:p-8 shadow-md space-y-6">
                <h3 className="text-lg font-black text-[#010E62] border-b border-[#E8DDBF]/60 pb-3 flex items-center justify-between">
                  ...
                  <Link href="/effy_edu_management_system/news-events" className="text-xs font-bold text-accent hover:underline">
                    View All →
                  </Link>
                </h3>
                ...
                <div className="space-y-4">
                  {relatedItems.map((related) => (
                    <Link
                      key={related.id}
                      href={`/effy_edu_management_system/news-events/${related.id}`}
                    ...
                    >
                      <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-slate-200 shrink-0">
                        <Image
                          src={related.imageUrl || "/effy_edu_management_system/images/gallery-event.png"}
                          alt={related.title}
                          fill
