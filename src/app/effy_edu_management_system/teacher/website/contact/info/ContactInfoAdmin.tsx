...
export default function ContactInfoAdmin({ initialSectionData }: { initialSectionData: any }) {
  ...
  const [address, setAddress] = useState(content.address || "Education Avenue, Dhaka, Bangladesh");
  const [transitInfo, setTransitInfo] = useState(content.transitInfo || "Conveniently located beside the central academic district in Dhaka. Easily accessible from all parts of the area by local transport (CNG/bus).");
  ...
  const [mapEmbedUrl, setMapEmbedUrl] = useState(content.mapEmbedUrl || "https://maps.google.com/maps?q=Dhaka%20Bangladesh&t=&z=16&ie=UTF8&iwloc=&output=embed");
  const [mapDirectionUrl, setMapDirectionUrl] = useState(content.mapDirectionUrl || "https://maps.google.com/?q=Dhaka+Bangladesh");
