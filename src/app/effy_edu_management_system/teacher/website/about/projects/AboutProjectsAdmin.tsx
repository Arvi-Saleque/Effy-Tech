...
import { updatePageSection } from "@/features/effy-edu-demo/features/website-cms/actions/content-actions";
import { projectsData as defaultData, ProjectItem, SectionHeader } from "@/features/effy-edu-demo/data/about";
import { IconPicker } from "@/features/effy-edu-demo/features/website-cms/components/IconPicker";
import { MediaSelector } from "@/features/effy-edu-demo/features/website-cms/components/MediaSelector";
import { SectionHeaderEditor } from "@/features/effy-edu-demo/features/website-cms/components/SectionHeaderEditor";
...
export default function AboutProjectsAdmin({ initialSectionData }: { initialSectionData: any }) {
  ...
  const handleResourceUpload = async (projectIndex: number, linkIndex: number, file: File) => {
    try {
      ...
      const { getPreSignedUploadUrl } = await import("@/app/effy_edu_management_system/actions/r2-upload");
      ...
      const resourceUrl = `/effy_edu_management_system/api/resource?key=${urlResult.r2Key}`;
                        ...
                        <div className="w-full flex flex-col sm:flex-row gap-2">
                          {link.isFile ? (
                            <div className="w-full sm:w-3/4 flex flex-col space-y-1">
                              {link.url && link.url.includes('/effy_edu_management_system/api/resource') ? (
