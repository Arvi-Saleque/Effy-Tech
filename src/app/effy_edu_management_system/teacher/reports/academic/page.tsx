...
import { StudentProgressReportFilters } from "@/features/effy-edu-demo/components/reports/student-progress-report-filters";
import { getStudentProgressReportDirectory } from "@/features/effy-edu-demo/lib/reports/student-progress-report-directory";
...
export default async function AcademicPerformanceReportPage({ searchParams }: PageProps) {
  const { destination, profile } = await resolveAuthenticatedDestination(undefined, "TEACHER");
  ...
  return (
    <div className="space-y-7">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/effy_edu_management_system/teacher/reports" className="inline-flex items-center gap-2 text-xs font-black text-slate-500 transition hover:text-primary">
          <ArrowLeft className="h-4 w-4" /> Reports center
        </Link>
        {selectedBatchId && (
          <Link href={`/effy_edu_management_system/teacher/academic/${selectedBatchId}`} className="rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-[10px] font-black uppercase tracking-wide text-blue-700 transition hover:border-blue-200">
