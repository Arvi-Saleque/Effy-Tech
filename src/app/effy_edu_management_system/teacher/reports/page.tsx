  ...
  return (
    <div className="space-y-8 text-xs font-bold text-primary max-w-full overflow-hidden">
      ...
      <div className="flex border-b border-border/40 overflow-x-auto pb-px scrollbar-thin">
        {[
          { id: "enrollment", label: "Enrollment Ledger", icon: <Users className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports?tab=enrollment" },
          { id: "payment", label: "Monthly Collections", icon: <CreditCard className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports?tab=payment" },
          { id: "examination", label: "Exams Analysis", icon: <GraduationCap className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports?tab=examination" },
          { id: "student", label: "Student Performance", icon: <Award className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports?tab=student" },
          { id: "batch", label: "Batch Performance", icon: <Layers className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports?tab=batch" },
          { id: "academic", label: "Subject Progress", icon: <BookOpenCheck className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports/academic" },
          { id: "progress-report", label: "Progress Report", icon: <Printer className="h-4 w-4" />, href: "/effy_edu_management_system/teacher/reports/student-progress" },
        ].map((tab) => (
          <Link
            key={tab.id}
            href={tab.href}
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <form method="GET" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 flex-1">
                ...
                <div className="sm:col-span-2 md:col-span-5 flex justify-end gap-2 pt-2">
                  ...
                  <Link
                    href={`/effy_edu_management_system/api/reports/export?tab=enrollment&batchId=${tab1BatchId}&status=${tab1Status}&accountStatus=${tab1AccountStatus}&startDate=${tab1StartDate}&endDate=${tab1EndDate}`}
                    target="_blank"
                    ...
                    {enrollmentsData.map((e, index) => {
                      ...
                      return (
                        <tr key={index} className="hover:bg-slate-50/30">
                          <td className="py-3">
                            {stud ? (
                              <Link href={`/effy_edu_management_system/teacher/students/${stud.id}/edit`} className="font-extrabold text-indigo-600 hover:underline">
                                {stud.student_code}
                              </Link>
                            ) : "—"}
                          </td>
          ...
          <div className="bg-white p-5 rounded-2xl border border-border/40 shadow-sm space-y-4">
            <form method="GET" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div className="sm:col-span-2 md:col-span-5 flex justify-end gap-2 pt-2">
                ...
                <Link
                  href={`/effy_edu_management_system/api/reports/export?tab=payment&month=${tab2Month}&year=${tab2Year}&batchId=${tab2BatchId}&status=${tab2Status}&paymentMethod=${tab2Method}`}
                  target="_blank"
                  className="px-4 py-2 border border-border bg-white hover:bg-slate-50 text-primary rounded-xl font-extrabold flex items-center gap-1.5 shadow-sm transition-all"
                >
                  <Download className="h-3.5 w-3.5" />
          ...
          <div className="bg-white p-5 rounded-2xl border border-border/40 shadow-sm space-y-4">
            <form method="GET" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div className="sm:col-span-2 md:col-span-5 flex justify-end gap-2 pt-2">
                ...
                <Link
                  href={`/effy_edu_management_system/api/reports/export?tab=examination&batchId=${tab3BatchId}&examType=${tab3ExamType}&pubStatus=${tab3PubStatus}&startDate=${tab3StartDate}&endDate=${tab3EndDate}`}
                  target="_blank"
                  className="px-4 py-2 border border-border bg-white hover:bg-slate-50 text-primary rounded-xl font-extrabold flex items-center gap-1.5 shadow-sm transition-all"
                >
                  <Download className="h-3.5 w-3.5" />
                  ...
                  <tbody className="divide-y divide-border/10">
                    {examsData.map((e, index) => {
                      ...
                      return (
                        <tr key={index} className="hover:bg-slate-50/30">
                          <td className="py-3">
                            <Link href={`/effy_edu_management_system/teacher/exams/${e.id}`} className="font-extrabold text-indigo-600 hover:underline">
                              {e.name}
                            </Link>
                          </td>
                          <td className="py-3 text-slate-700">{b ? `${b.name} (${b.code})` : "—"}</td>
