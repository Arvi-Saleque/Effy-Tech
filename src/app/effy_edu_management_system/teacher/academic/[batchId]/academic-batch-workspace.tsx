...
} from "@/app/effy_edu_management_system/actions/academic-management";
import {
  subjectStatuses,
  subjectThemeKeys,
  subjectUnitStatuses,
...
} from "@/features/effy-edu-demo/lib/validations/academic";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/effy_edu_management_system/teacher/academic" className="inline-flex items-center gap-2 text-xs font-black text-slate-500 transition hover:text-primary">
          <ArrowLeft className="h-4 w-4" />
          Academic Control Center
        </Link>
        <Link href={`/effy_edu_management_system/teacher/batches/${batch.id}`} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-[10px] font-black uppercase tracking-wide text-slate-600 transition hover:border-blue-200 hover:text-blue-700">
            ...
            <div className="space-y-5">
              ...
              <section className="rounded-3xl border border-border/60 bg-white p-5 shadow-sm sm:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Link
                    href={`/effy_edu_management_system/teacher/reports/academic?batchId=${batch.id}&subjectId=${selectedSubject.id}`}
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  <div className="rounded-2xl border border-amber-100 bg-amber-50/55 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Link href={`/effy_edu_management_system/teacher/batches/${batch.id}/materials?subjectId=${selectedSubject.id}`} className="rounded-lg bg-white px-3 py-2 text-[10px] font-black text-amber-700 shadow-sm transition hover:bg-amber-100">Manage resources</Link>
                      <Link href={`/effy_edu_management_system/teacher/materials/new?batchId=${batch.id}&subjectId=${selectedSubject.id}`} className="rounded-lg border border-amber-200 px-3 py-2 text-[10px] font-black text-amber-700 transition hover:bg-amber-100">Add material</Link>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-blue-100 bg-blue-50/55 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Link href={`/effy_edu_management_system/teacher/assignments?batchId=${batch.id}&subjectId=${selectedSubject.id}`} className="rounded-lg bg-white px-3 py-2 text-[10px] font-black text-blue-700 shadow-sm transition hover:bg-blue-100">View assignments</Link>
                      <Link href={`/effy_edu_management_system/teacher/assignments/new?batchId=${batch.id}&subjectId=${selectedSubject.id}`} className="rounded-lg border border-blue-200 px-3 py-2 text-[10px] font-black text-blue-700 transition hover:bg-blue-100">Create assignment</Link>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-violet-100 bg-violet-50/55 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Link href={`/effy_edu_management_system/teacher/routine?batchId=${batch.id}&subjectId=${selectedSubject.id}`} className="rounded-lg bg-white px-3 py-2 text-[10px] font-black text-violet-700 shadow-sm transition hover:bg-violet-100">View routine</Link>
                      <Link href={`/effy_edu_management_system/teacher/routine/new?batchId=${batch.id}&subjectId=${selectedSubject.id}`} className="rounded-lg border border-violet-200 px-3 py-2 text-[10px] font-black text-violet-700 transition hover:bg-violet-100">Schedule class</Link>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="rounded-3xl border border-border/60 bg-white p-5 shadow-sm sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Link href={`/effy_edu_management_system/teacher/exams/new?batchId=${batch.id}&subjectId=${selectedSubject.id}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-100 bg-violet-50 px-4 py-2.5 text-xs font-black text-violet-700 transition hover:bg-violet-100"><Plus className="h-4 w-4" /> Schedule exam</Link>
                </div>
                ...
                {selectedSubject.exams.length === 0 ? (
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2">
                    {selectedSubject.exams.map((exam) => (
                      <Link key={exam.id} href={`/effy_edu_management_system/teacher/exams/${exam.id}`} className="group rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-violet-200 hover:bg-violet-50/35 hover:shadow-sm">
                        <div className="flex items-start justify-between gap-3">
