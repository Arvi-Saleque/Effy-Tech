...
import { RoutineFilters } from "@/features/effy-edu-demo/components/class-routine/routine-filters";
import { ClassSessionStatusActions } from "@/features/effy-edu-demo/components/class-routine/class-session-status-actions";
...
import type { ClassSessionStatus } from "@/features/effy-edu-demo/lib/validations/class-routine";
import { classSessionStatuses } from "@/features/effy-edu-demo/lib/validations/class-routine";
  ...
  return (
    <div className="space-y-7">
      <DashboardPageHeader
        ...
        actions={
          <Link href="/effy_edu_management_system/teacher/routine/new" className="primary-btn inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-black">
      ...
      {sessions.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center">
          ...
          <Link href="/effy_edu_management_system/teacher/routine/new" className="primary-btn mt-5 inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-xs font-black"><Plus className="h-4 w-4" /> Schedule class</Link>
        </div>
      ) : (
            ...
            return (
              <article key={session.id} className="rounded-3xl border border-border/60 bg-white p-5 shadow-sm transition hover:border-blue-200 hover:shadow-xl hover:shadow-blue-950/8">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Link href={`/effy_edu_management_system/teacher/routine/${session.id}/edit`} aria-label={`Edit ${session.title}`} className="rounded-xl bg-slate-50 p-2 text-slate-400 transition hover:bg-blue-50 hover:text-blue-700"><ArrowRight className="h-4 w-4" /></Link>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  ...
                  <Link href={`/effy_edu_management_system/teacher/routine/${session.id}/edit`} className="text-[10px] font-black text-blue-700 hover:underline">Edit details</Link>
                </div>
              </article>
            );
          })}
