...
} from "@/features/effy-edu-demo/lib/finance/finance-domain";
...
} from "@/features/effy-edu-demo/components/finance/expense-controls";
import {
  FinanceCharts,
  type FinanceCategoryChartPoint,
  type FinanceMonthlyChartPoint,
} from "@/features/effy-edu-demo/components/finance/finance-charts";
import { FINANCE_PAYMENT_METHODS } from "@/features/effy-edu-demo/lib/validations/finance";

const DEMO_HOME = "/effy_edu_management_system";
const FINANCE_HOME = `${DEMO_HOME}/teacher/finance`;
...
export default async function FinanceManagementPage({
  ...
  const margin =
    currentSummary.income > 0
      ? (currentSummary.netProfit / currentSummary.income) * 100
      : 0;
  ...
  trendExpenses.forEach((item) => {
    ...
    if (entry && item.status === "POSTED") {
      entry.expense += Number(item.amount);
    }
  });
  const monthlyData: FinanceMonthlyChartPoint[] = Array.from(
    monthlyMap.values()
  ).map((entry) => ({
    ...entry,
  ...
  currentExpenses
    ...
    .forEach((expense) => {
      categoryTotals.set(
        expense.category_id,
        (categoryTotals.get(expense.category_id) || 0) +
  ...
  previousExpenses
    ...
    .forEach((expense) => {
      previousCategoryTotals.set(
        expense.category_id,
        (previousCategoryTotals.get(expense.category_id) || 0) +
          Number(expense.amount)
      );
    });
  ...
  const unifiedEntries = [
    ...currentIncome.map((item) => ({
      ...
      href: `${DEMO_HOME}/teacher/payments/${item.payment_id}`,
      voided: false,
    })),
    ...currentExpenses.map((item) => ({
      ...
      detail: item.payee
        ? `Paid to ${item.payee}`
        : item.description || "Operating expense",
  ...
  const exportHref = `${DEMO_HOME}/api/finance/export?${exportParams.toString()}`;

  return (
    <div className="space-y-6 pb-10 print:bg-white">
      <DashboardPageHeader
        title="Finance Management"
        description="Actual fee collections and operating expenses in one synchronized, auditable demo ledger."
        actions={
      ...
      <section className="rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 print:hidden">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] font-semibold leading-5 text-amber-900/80">
              All collections, expenses, receipts and changes are local mock
              data. They reset when the demo server restarts.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-primary/10 bg-gradient-to-br from-[#071A45] via-primary to-[#132B6B] p-5 text-white shadow-xl shadow-primary/10 sm:p-6 print:hidden">
        <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-xs font-semibold leading-5 text-blue-100">
              Income is synchronized from confirmed student collections.
              Expected fees and unpaid dues are never counted as income.
            </p>
          </div>
        ...
        <form
          action={FINANCE_HOME}
          className="mt-5 grid gap-3 border-t border-white/15 pt-5 sm:grid-cols-[1fr_1fr_auto]"
        >
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard
          ...
          change={profitChange}
          icon={
            currentSummary.netProfit >= 0 ? TrendingUp : TrendingDown
          }
          tone="navy"
        />
        <SummaryCard
      ...
      <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 px-4 py-3 text-xs font-semibold text-emerald-900 print:hidden">
        <span className="font-black">Income is automatic.</span> Record or
        correct student fees in{" "}
        <Link
          href={`${DEMO_HOME}/teacher/payments`}
          className="font-black underline underline-offset-2"
        >
          Payment Ledger
        </Link>
        ; this dashboard reads the same generalized payment records.
      </div>
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            expense.category?.color_hex || "#64748B",
                        }}
                      />
                      <p
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
                  <div className="flex items-center gap-2">
                    {expense.receipt_storage_path && (
                      <a
                        href={`${DEMO_HOME}/api/finance/expenses/${expense.id}/receipt`}
                        target="_blank"
                        rel="noreferrer"
            ...
            {categoryData.map((item) => {
              ...
              const share =
                currentSummary.expense > 0
                  ? (item.value / currentSummary.expense) * 100
                  : 0;
              return (
                <div
                  key={item.name}
