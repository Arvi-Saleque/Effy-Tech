...
import { DashboardCard } from "@/features/effy-edu-demo/components/dashboard/dashboard-card";
...
export default async function TeacherDashboardPage() {
  ...
  const { data: allBatches } = await supabase
    .from("batches")
    .select("id, status, admission_open");

  const totalBatches = allBatches?.length || 0;
  const runningBatches = allBatches?.filter((b) => b.status === "RUNNING").length || 0;
  const openAdmissionBatches = allBatches?.filter((b) => b.admission_open && b.status !== "ARCHIVED").length || 0;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <h4 className="text-xs uppercase tracking-wider text-muted font-extrabold border-b pb-1 flex justify-between items-center">
              ...
              <Link href="/effy_edu_management_system/teacher/students" className="text-[10px] text-primary hover:text-accent font-extrabold flex items-center">
                <span>View All</span>
              ...
              <div className="space-y-2">
                {pendingApprovalsList.map((stud) => (
                  <div key={stud.id} className="p-3 bg-slate-50/40 border border-slate-100 rounded-xl flex justify-between items-center text-xs">
                    ...
                    <Link href={`/effy_edu_management_system/teacher/students/${stud.id}`} className="px-2.5 py-1 bg-primary text-white text-[10px] font-black rounded-lg hover:bg-primary-dark transition-all">
                      Review
                    </Link>
                  </div>
                ))}
          ...
          <div className="space-y-3">
            <h4 className="text-xs uppercase tracking-wider text-muted font-extrabold border-b pb-1 flex justify-between items-center">
              ...
              <Link href="/effy_edu_management_system/teacher/payments" className="text-[10px] text-primary hover:text-accent font-extrabold flex items-center">
                <span>Ledger</span>
                <ChevronRight className="h-3 w-3" />
              </Link>
            </h4>
                ...
                {unpaidInvoicesList.map((pay) => (
                  <div key={pay.id} className="p-3 bg-slate-50/40 border border-slate-100 rounded-xl flex justify-between items-center text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link href={`/effy_edu_management_system/teacher/payments/${pay.id}`} className="px-2.5 py-1 bg-slate-100 text-[10px] border font-black rounded-lg hover:bg-slate-200 transition-all">
                        Record
                      </Link>
                    </div>
                  </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs uppercase tracking-wider text-muted font-extrabold border-b pb-1 flex justify-between items-center">
              ...
              <Link href="/effy_edu_management_system/teacher/exams" className="text-[10px] text-primary hover:text-accent font-extrabold flex items-center">
              ...
              <div className="space-y-2">
                {resultDraftsList.map((exam) => (
                  <div key={exam.id} className="p-3 bg-slate-50/40 border border-slate-100 rounded-xl flex justify-between items-center text-xs">
                    ...
                    <Link href={`/effy_edu_management_system/teacher/exams/${exam.id}`} className="px-2.5 py-1 bg-amber-600 text-white text-[10px] font-black rounded-lg hover:bg-amber-700 transition-all">
                      Publish
                    </Link>
                  </div>
                ))}
          ...
          <div className="space-y-3 lg:col-span-2">
            <h4 className="text-xs uppercase tracking-wider text-muted font-extrabold border-b pb-1 flex justify-between items-center">
              ...
              <Link href="/effy_edu_management_system/teacher/enrollments" className="text-[10px] text-primary hover:text-accent font-extrabold flex items-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {disabledEnrollments.map((enr) => (
                  <div key={enr.id} className="p-3 bg-rose-50/20 border border-rose-100 rounded-xl flex justify-between items-start text-xs">
                    ...
                    <Link href={`/effy_edu_management_system/teacher/students/${(enr.student as any)?.id}`} className="px-2 py-0.5 border text-[9px] font-bold rounded hover:bg-slate-50">Manage</Link>
                  </div>
                ))}
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-border/40 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-center text-xs">
            <Link href="/effy_edu_management_system/teacher/batches/new" className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-primary hover:text-white transition-colors flex flex-col items-center gap-1">
              <Plus className="h-4 w-4 text-muted group-hover:text-white" />
              <span>Add Batch</span>
            </Link>
            <Link href="/effy_edu_management_system/teacher/students" className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-primary hover:text-white transition-colors flex flex-col items-center gap-1">
              <Search className="h-4 w-4 text-muted group-hover:text-white" />
              <span>Find Student</span>
            </Link>
            <Link href="/effy_edu_management_system/teacher/payments/new" className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-primary hover:text-white transition-colors flex flex-col items-center gap-1">
              <CreditCard className="h-4 w-4 text-muted group-hover:text-white" />
              <span>Record Payment</span>
            </Link>
            <Link href="/effy_edu_management_system/teacher/materials/new" className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-primary hover:text-white transition-colors flex flex-col items-center gap-1">
              <FileText className="h-4 w-4 text-muted group-hover:text-white" />
              <span>Upload Material</span>
            </Link>
            <Link href="/effy_edu_management_system/teacher/exams/new" className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-primary hover:text-white transition-colors flex flex-col items-center gap-1">
              <GraduationCap className="h-4 w-4 text-muted group-hover:text-white" />
              <span>New Exam</span>
            </Link>
            <Link href="/effy_edu_management_system/teacher/reports" className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-primary hover:text-white transition-colors flex flex-col items-center gap-1">
