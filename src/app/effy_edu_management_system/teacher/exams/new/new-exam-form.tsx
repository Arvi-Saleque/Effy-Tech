...
import { createExamAction } from "@/app/effy_edu_management_system/actions/exams";
  ...
  const onSubmit = async (data: ExamFormOutput) => {
    ...
    try {
      ...
      if (!res.success) {
        ...
        router.push(initialBatchId ? `/effy_edu_management_system/teacher/academic/${data.batchId}` : "/effy_edu_management_system/teacher/exams");
        router.refresh();
      }
    } catch (error: unknown) {
