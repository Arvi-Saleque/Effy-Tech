  ...
  return (
    <div className="space-y-8 text-xs font-bold text-primary">
      ...
      <DashboardPageHeader
        ...
        actions={
          <Link
            href="/effy_edu_management_system/teacher/exams/new"
                  ...
                  return (
                    <tr key={exam.id} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        <Link
                          href={`/effy_edu_management_system/teacher/exams/${exam.id}`}
                        ...
                        <span className="text-[10px] font-bold text-muted/70 block mt-0.5">
                          Batch: <Link href={`/effy_edu_management_system/teacher/batches/${exam.batch_id}`} className="hover:underline text-primary">{exam.batches?.name} ({exam.batches?.code})</Link>
                        </span>
                      </td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-border/30 bg-slate-50/50">
            ...
            <div className="flex gap-2">
              <Link
                href={`/effy_edu_management_system/teacher/exams?page=${page - 1}&search=${search}&batchId=${batchId}&examType=${examType}&status=${status}&startDate=${startDate}&endDate=${endDate}`}
