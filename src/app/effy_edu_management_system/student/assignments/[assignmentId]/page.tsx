...
import { StudentAssignmentSubmissionForm } from "@/features/effy-edu-demo/components/assignments/student-assignment-submission-form";
import { requireActiveEnrollment } from "@/features/effy-edu-demo/lib/auth-guards";
...
export default async function StudentAssignmentDetailPage({
  ...
  const [{ assignmentId }, { destination, studentProfile }] = await Promise.all([
    params,
    resolveAuthenticatedDestination(undefined, "STUDENT"),
  ]);
  ...
  if (destination !== "STUDENT_DASHBOARD" || !studentProfile) redirect("/effy_edu_management_system/login?error=invalid_profile");
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 pb-12">
      <Link href="/effy_edu_management_system/student/assignments" className="inline-flex items-center gap-2 text-xs font-black text-slate-500 transition hover:text-blue-700"><ArrowLeft className="h-4 w-4" /> My assignments</Link>
