...
export default async function StudentAssignmentsPage() {
  const { destination, studentProfile } = await resolveAuthenticatedDestination(undefined, "STUDENT");
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {assignments.map((assignment) => {
              ...
              return (
                <Link key={assignment.id} href={`/effy_edu_management_system/student/assignments/${assignment.id}`} className="group rounded-3xl border border-border/60 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-950/8">
