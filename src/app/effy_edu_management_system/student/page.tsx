...
import { StatusBadge } from "@/features/effy-edu-demo/components/dashboard/status-badge";
import { formatCurrency } from "@/features/effy-edu-demo/lib/currency";
import { normalizeSchedule } from "@/features/effy-edu-demo/lib/schedule";
import {
  Calendar,
  Bell,
  BookOpen,
...
function getNextClassInfo(activeEnrollments: any[]): {
  ...
  for (const enr of activeEnrollments) {
    ...
    if (!batch) continue;
    const schedule = normalizeSchedule(batch.schedule);

    if (schedule && schedule.days.length > 0 && schedule.time) {
      const isToday = schedule.days.some((day) => {
        const normalizedDay = day.toLowerCase();
        const normalizedCurrentDay = currentDayName.toLowerCase();
        return normalizedDay.includes(normalizedCurrentDay) || normalizedCurrentDay.includes(normalizedDay);
      });

      // Extract the start time for display while remaining safe for non-string seed values.
      const timeParts = schedule.time.split("-").map((time) => time.trim());
      const startTime = timeParts[0] || schedule.time;

      return {
        text: isToday ? `Today, ${startTime}` : `${schedule.days[0]}, ${startTime}`,
        batchName: String(batch.name || "Upcoming class"),
        scheduleDays: schedule.daysText,
...
export default async function StudentDashboardPage() {
  const { profile, studentProfile, destination } = await resolveAuthenticatedDestination(undefined, "STUDENT");

  if (destination === "UNAUTHENTICATED") redirect("/effy_edu_management_system/login");
  if (destination === "PENDING_APPROVAL") redirect("/effy_edu_management_system/pending-approval");
  if (destination === "ACCOUNT_DISABLED") redirect("/effy_edu_management_system/account-disabled");
  ...
  return (
    <div className="space-y-8 text-xs font-bold text-primary max-w-[1500px] mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3.5 sm:gap-4">
        ...
        <Link href="/effy_edu_management_system/student/payments" className="bg-rose-50/90 border border-rose-200/80 p-4 rounded-2xl flex items-center gap-3.5 shadow-2xs hover:shadow-sm transition-all group min-w-0">
        ...
        <Link href="/effy_edu_management_system/student/notifications" className="bg-purple-50/90 border border-purple-200/80 p-4 rounded-2xl flex items-center gap-3.5 shadow-2xs hover:shadow-sm transition-all group min-w-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5">
        ...
        <Link href="/effy_edu_management_system/student/profile" className="bg-white border border-border/60 p-5 sm:p-6 rounded-3xl shadow-xs hover:shadow-md transition-all group flex flex-col justify-between min-w-0 overflow-hidden">
        ...
        <Link href="/effy_edu_management_system/student/notifications" className="bg-white border border-border/60 p-5 sm:p-6 rounded-3xl shadow-xs hover:shadow-md transition-all group flex flex-col justify-between min-w-0 overflow-hidden">
          <div className="flex items-center justify-between">
        ...
        <Link href="/effy_edu_management_system/student/payments" className="bg-white border border-border/60 p-5 sm:p-6 rounded-3xl shadow-xs hover:shadow-md transition-all group flex flex-col justify-between min-w-0 overflow-hidden">
          <div className="flex items-center justify-between">
      ...
      <section className="overflow-hidden rounded-3xl border border-blue-100 bg-[linear-gradient(135deg,#F8FBFF,#EEF4FF)] p-5 shadow-xs sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link href="/effy_edu_management_system/student/assignments" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#102A66] px-4 py-2.5 text-xs font-black text-white transition hover:bg-[#173B89]">Open assignments <ChevronRight className="h-4 w-4" /></Link>
        </div>
        {pendingAssignments.length > 0 && (
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {pendingAssignments.slice(0, 3).map((assignment) => {
              ...
              return (
                <Link key={assignment.id} href={`/effy_edu_management_system/student/assignments/${assignment.id}`} className="rounded-2xl border border-white bg-white/85 p-4 transition hover:border-blue-200 hover:shadow-sm">
        ...
        <div className="lg:col-span-2 space-y-8">
                  ...
                  return (
                    <div key={enr.id} className="p-4 sm:p-5 border border-slate-200/80 rounded-2xl bg-white flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition-all group min-w-0">
                      <div className="space-y-3.5 min-w-0">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-3 border-t border-slate-100">
                          ...
                          <div className="bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 min-w-0">
                            ...
                            <span className="text-xs font-extrabold text-slate-800 block mt-1 truncate" title={schedule?.daysText || "Not configured"}>
                              {schedule?.daysText || "Not configured"}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-5 pt-3">
                        <Link
                          href={`/effy_edu_management_system/student/batches/${enr.batch_id}`}
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              ...
              <Link href="/effy_edu_management_system/student/exams" className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 shrink-0">
                <span>View All</span>
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              ...
              <Link href="/effy_edu_management_system/student/results" className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 self-start sm:self-auto">
                        ...
                        return (
                          <tr key={r.id} className="hover:bg-slate-50/80 transition-colors group">
                            ...
                            <td className="py-3.5 text-right pr-2">
                              <Link
                                href={`/effy_edu_management_system/student/batches/${r.exam.batch_id}/exams/${r.exam.id}`}
        ...
        <div className="space-y-6 sm:space-y-8 min-w-0">
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-4 min-w-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 gap-2.5 sm:gap-3 text-center text-xs">
              <Link href="/effy_edu_management_system/student/profile" className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href="/effy_edu_management_system/student/payments" className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href="/effy_edu_management_system/student/results" className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href="/effy_edu_management_system/student/exams" className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href="/effy_edu_management_system/student/academics" className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href="/effy_edu_management_system/class-routine" className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            ...
            <Link
              href="/effy_edu_management_system/student/payments"
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-4 min-w-0">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              <Link href="/effy_edu_management_system/student/profile" className="text-xs font-bold text-primary hover:text-accent shrink-0">
                View All
              </Link>
            </div>
                ...
                {recentMaterials.map((m) => {
                  ...
                  return (
                    <div key={m.id} className="p-3.5 bg-slate-50/70 border border-slate-200/70 rounded-2xl flex items-center justify-between gap-3 hover:border-slate-300 transition-all group min-w-0">
                      ...
                      <Link
                        href={`/effy_edu_management_system/student/batches/${m.batch_id}/materials`}
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-4 min-w-0">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              <Link href="/effy_edu_management_system/student/notifications" className="text-xs font-bold text-primary hover:text-accent shrink-0">
                View All
              </Link>
            </div>
