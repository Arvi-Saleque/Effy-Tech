...
import { normalizeSchedule, type NormalizedSchedule } from "@/features/effy-edu-demo/lib/schedule";
...
function getNextClassDisplay(schedule: NormalizedSchedule | null): { dayText: string; timeText: string; isToday: boolean } | null {
  if (!schedule || schedule.days.length === 0 || !schedule.time) return null;
  ...
  return {
    dayText: isToday ? "Today" : schedule.days[0],
...
export default async function StudentBatchDetailsPage({ params }: PageProps) {
  ...
  if (destination === "INVALID_PROFILE") redirect("/effy_edu_management_system/login?error=invalid_profile");
  ...
  if (profile?.role === "STUDENT") {
    if (!studentProfile) redirect("/effy_edu_management_system/login?error=invalid_profile");
  ...
  return (
    <div className="space-y-8 text-xs font-bold text-primary max-w-[1500px] mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[10px] sm:text-xs font-black uppercase tracking-wider text-muted">
        <Link href="/effy_edu_management_system/student" className="hover:text-primary transition-colors">PORTAL</Link>
        <span>&bull;</span>
      ...
      <div className="bg-white p-5 sm:p-8 rounded-3xl border border-border/60 shadow-xs space-y-6 sm:space-y-7 min-w-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:flex xl:items-center gap-3 w-full xl:w-auto">
            <div className="bg-slate-50/90 border border-slate-200/80 p-3 rounded-2xl flex items-center gap-3 shrink-0 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <span className="text-xs font-black text-slate-900 truncate block" title={schedule?.daysText || "Not configured"}>{schedule?.daysText || "Not configured"}</span>
              </div>
            </div>
            ...
            <Link href={`/effy_edu_management_system/student/batches/${batchId}/payments`} className="bg-slate-50/90 hover:bg-rose-50/80 border border-slate-200/80 hover:border-rose-200 p-3 rounded-2xl flex items-center gap-3 shrink-0 transition-all group min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4 pt-1">
          {/* Stat 1 */}
          <Link href={`/effy_edu_management_system/student/batches/${batchId}/academics`} className="bg-slate-50/60 hover:bg-slate-100/80 border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 transition-all group min-w-0">
          ...
          {/* Stat 2 */}
          <Link href={`/effy_edu_management_system/student/batches/${batchId}/payments`} className="bg-slate-50/60 hover:bg-slate-100/80 border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 transition-all group min-w-0">
          ...
          {/* Stat 3 */}
          <Link href={`/effy_edu_management_system/student/batches/${batchId}/exams`} className="bg-slate-50/60 hover:bg-slate-100/80 border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 transition-all group min-w-0">
          ...
          {/* Stat 4 */}
          <Link href={`/effy_edu_management_system/student/batches/${batchId}/materials`} className="bg-slate-50/60 hover:bg-slate-100/80 border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 transition-all group min-w-0">
        ...
        <div className="bg-gradient-to-r from-[#0A192F] via-slate-900 to-indigo-950 p-5 sm:p-7 rounded-3xl text-white shadow-md relative overflow-hidden border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-5 min-w-0">
          <div className="space-y-2.5 z-10 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs !text-slate-200 font-semibold mt-1.5 leading-relaxed break-words" style={{ color: '#E2E8F0' }}>
                Schedule Days: <span className="!text-white font-black underline decoration-blue-400 decoration-2" style={{ color: '#FFFFFF' }}>{schedule?.daysText || "N/A"}</span> &bull; Subject: <span className="!text-amber-300 font-black" style={{ color: '#FCD34D' }}>{batch.subject || "General"}</span>
              </p>
            </div>
          </div>
          ...
          <div className="z-10 shrink-0 self-start sm:self-center">
            <Link
              href="/effy_edu_management_system/class-routine"
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/exams`} className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 shrink-0">
              ...
              <div className="space-y-3.5">
                {upcomingExams.map((exam) => {
                  ...
                  return (
                    <Link
                      key={exam.id}
                      href={`/effy_edu_management_system/student/batches/${batchId}/exams`}
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/materials`} className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 self-start sm:self-auto shrink-0">
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/results`} className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 self-start sm:self-auto shrink-0">
                      ...
                      return (
                        <tr key={r.id} className="hover:bg-slate-50/80 transition-colors group">
                          <td className="py-3.5 pl-2 font-black text-slate-900 group-hover:text-primary transition-colors max-w-[160px] truncate" title={r.exam.name}>
                            <Link href={`/effy_edu_management_system/student/batches/${batchId}/exams/${r.exam.id}`}>
                              {r.exam.name}
                            </Link>
                          </td>
        ...
        <div className="space-y-6 sm:space-y-8 min-w-0">
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-4 min-w-0">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/announcements`} className="text-xs font-bold text-primary hover:text-accent shrink-0">
                View All
              </Link>
            </div>
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/payments`} className="text-xs font-bold text-primary hover:text-accent shrink-0">
                View History
              </Link>
            </div>
            ...
            <Link
              href={`/effy_edu_management_system/student/batches/${batchId}/payments`}
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-5 min-w-0">
            ...
            <Link
              href={`/effy_edu_management_system/student/batches/${batchId}/academics`}
          ...
          <div className="bg-white p-5 sm:p-7 rounded-3xl border border-border/60 shadow-xs space-y-4 min-w-0">
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:gap-3 text-center text-xs">
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/materials`} className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/exams`} className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/results`} className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
              ...
              <Link href={`/effy_edu_management_system/student/batches/${batchId}/payments`} className="p-3.5 sm:p-4 border border-slate-200/80 rounded-2xl bg-slate-50/60 hover:bg-[#0A192F] hover:text-white transition-all flex flex-col items-center justify-center gap-2 group shadow-2xs min-w-0">
