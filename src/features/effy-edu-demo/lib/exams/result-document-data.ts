...
export async function getExamResultDocumentData(examId: string): Promise<ResultDocumentData | null> {
  ...
  const branding = {
    title: settings.coachingName || settings.coachingCenterName || "EDUPILOT COACHING ACADEMY",
    ...
    email: settings.email || "hello@edupilot.demo",
    address: settings.address || "Dhaka, Bangladesh",
    teacherName: settings.teacherName || "Dr. Arif Rahman",
  };
