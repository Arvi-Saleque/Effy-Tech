...
export async function resolveAuthenticatedDestination(
  ...
  if (roleHint) {
    const { demoTables, getDemoUser } = await import("@/features/effy-edu-demo/lib/demo/mock-data");
    if (roleHint === "TEACHER") {
      const profile = demoTables.profiles.find((row: any) => row.role === "TEACHER") || null;
      return { destination: "TEACHER_DASHBOARD", profile, studentProfile: null, user: getDemoUser("TEACHER") };
    }
    const studentProfile = demoTables.student_profiles[0] || null;
    const profile = studentProfile ? demoTables.profiles.find((row: any) => row.id === studentProfile.profile_id) || null : null;
    return { destination: "STUDENT_DASHBOARD", profile, studentProfile, user: getDemoUser("STUDENT") };
  }

  const supabase = supabaseOverride ?? (await (await import("./server")).createClient());
