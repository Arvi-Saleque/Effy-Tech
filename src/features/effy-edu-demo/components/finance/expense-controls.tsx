...
} from "@/app/effy_edu_management_system/actions/finance";
...
const labelClass =
  "text-[11px] font-black uppercase tracking-wider text-slate-600";
...
async function uploadReceipt(file: File) {
  ...
  if (!result.success || !result.uploadUrl || !result.storagePath) {
    throw new Error(
      result.message || "Could not initialize receipt upload."
    );
  }
    ...
    startTransition(async () => {
      try {
        ...
        const input = {
          ...
          ) as (typeof FINANCE_PAYMENT_METHODS)[number],
        ...
        if (!result.success) {
          throw new Error(result.message || "Expense could not be saved.");
        }
            ...
            <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur sm:px-7">
              <div>
                ...
                <p className="mt-1 text-xs font-semibold text-muted">
                  Demo changes are process-local and reset when the demo
                  restarts.
                </p>
              </div>
              <button
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[10px] font-semibold text-muted">
                      PDF, JPG, PNG or WebP · maximum 10 MB · local demo
                      storage
                    </p>
...
export function ExpenseStatusAction({
  ...
  function changeStatus() {
    if (expense.status === "VOID") {
      if (
        !window.confirm(
          "Restore this expense and include it in financial totals again?"
        )
      ) {
        return;
      }
