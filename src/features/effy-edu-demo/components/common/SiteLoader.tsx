...
export function SiteLoader({ isDismissing = false, message }: SiteLoaderProps) {
  ...
  const title = settings?.coachingCenterName || "EduPilot Coaching Academy";
  const tagline =
    settings?.tagline ||
    settings?.heroDescription ||
    "Physics & Higher Mathematics Admission Care";
  const logoUrl = settings?.logoUrl || "/effy_edu_management_system/images/edupilot-logo.svg";
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      setProgress((current) => {
        ...
        if (current < 65) return Math.min(99, current + Math.floor(Math.random() * 6) + 5);
        if (current < 94) return Math.min(99, current + Math.floor(Math.random() * 3) + 2);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none select-none">
        <img
          src="/effy_edu_management_system/images/bg.png"
