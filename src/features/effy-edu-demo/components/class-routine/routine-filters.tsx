...
export function RoutineFilters({
  ...
  function apply() {
    ...
    router.push(`/effy_edu_management_system/teacher/routine${params.size ? `?${params.toString()}` : ""}`);
  }
  ...
  return (
    <section className="rounded-3xl border border-border/60 bg-white p-4 shadow-sm sm:p-5">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_.8fr_1.2fr_auto_auto]">
        ...
        <button type="button" onClick={() => { setBatchId(""); setSubjectId(""); setStatus(""); setQuery(""); router.push("/effy_edu_management_system/teacher/routine"); }} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-slate-600 hover:bg-slate-50"><RotateCcw className="h-4 w-4" /> Reset</button>
      </div>
    </section>
  );
}
