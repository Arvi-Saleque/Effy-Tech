...
import { createMaterialAction, updateMaterialAction } from "@/app/effy_edu_management_system/actions/materials";
    ...
    if (isR2Upload) {
      ...
      try {
        ...
        startTransition(async () => {
          ...
          if (result.success) {
            router.push(`/effy_edu_management_system/teacher/materials`);
            router.refresh();
          } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 max-w-2xl mx-auto space-y-6 text-xs font-bold text-slate-800">
      ...
      <div className="pt-4 border-t border-slate-50 flex justify-end gap-3">
        <Link
          href={`/effy_edu_management_system/teacher/materials`}
