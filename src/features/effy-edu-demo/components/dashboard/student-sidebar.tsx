...
export function StudentSidebar({ className, onLinkClick, activeBatches = [] }: SidebarProps) {
  ...
  const navItems: NavItem[] = [
    { label: "Dashboard", href: "/effy_edu_management_system/student", icon: LayoutDashboard },
    { label: "Academic Journey", href: "/effy_edu_management_system/student/academics", icon: BookOpenCheck },
    { label: "Class Routine", href: "/effy_edu_management_system/student/routine", icon: CalendarDays },
    { label: "Assignments", href: "/effy_edu_management_system/student/assignments", icon: ClipboardList },
  ];
  ...
  activeBatches.forEach((batch) => {
    navItems.push({
      ...
      href: `/effy_edu_management_system/student/batches/${batch.id}`,
      icon: BookOpen,
    });
  });

  navItems.push(
    { label: "Exams & Results", href: "/effy_edu_management_system/student/exams", icon: GraduationCap },
    { label: "Payments", href: "/effy_edu_management_system/student/payments", icon: CreditCard },
    { label: "Notifications", href: "/effy_edu_management_system/student/notifications", icon: Bell },
    { label: "Profile", href: "/effy_edu_management_system/student/profile", icon: User },
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      router.push("/effy_edu_management_system/login");
      router.refresh();
    } catch (error) {
    ...
    <div className={cn("flex flex-col h-full bg-primary-dark text-white border-r border-slate-900", className)}>
      ...
      <nav className="flex-grow p-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          ...
          const isActive = item.href === "/effy_edu_management_system/student"
            ? pathname === "/effy_edu_management_system/student"
