...
export function AssignmentForm({
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      ...
      router.push(`/effy_edu_management_system/teacher/assignments/${result.entityId || initialData?.id}`);
      router.refresh();
    });
  }
  ...
  return (
    <form action={submit} className="space-y-6" aria-busy={pending}>
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
        <Link href={initialData ? `/effy_edu_management_system/teacher/assignments/${initialData.id}` : "/effy_edu_management_system/teacher/assignments"} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-xs font-black text-slate-600 transition hover:bg-slate-50">
