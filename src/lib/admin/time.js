...
/**
 * Calculates difference in calendar days between two YYYY-MM-DD date strings.
 * Safely parses them as noon UTC to avoid daylight saving or timezone shifts.
 */
export function calculateCalendarDays(startStr, endStr) {
  ...
  const startUtc = Date.UTC(sYear, sMonth - 1, sDay, 12, 0, 0);
  const endUtc = Date.UTC(eYear, eMonth - 1, eDay, 12, 0, 0);
